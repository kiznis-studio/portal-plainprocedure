...
export function getStateByAbbr(db: D1Database, abbr: string): Promise<State | null> {
...
export function getProcedureBySlug(db: D1Database, slug: string): Promise<Procedure | null> {
  return cached(`proc:${slug}`, async () => {
    const r = await db.prepare('SELECT * FROM procedures WHERE slug = ?1').bind(slug).first<Procedure>();
    return r;
  });
}

export function getProceduresByCategory(db: D1Database, category: string): Promise<Procedure[]> {
  return cached(`proc:cat:${category}`, async () => {
    const r = await db.prepare(
...
export function getTopProcedures(db: D1Database, limit: number = 10): Promise<Procedure[]> {
  return cached(`proc:top:${limit}`, async () => {
    const r = await db.prepare(
      'SELECT * FROM procedures ORDER BY national_total_services DESC LIMIT ?1'
    ).bind(limit).all<Procedure>();
    return r.results;
  });
}

export function getMostExpensiveProcedures(db: D1Database, limit: number = 10): Promise<Procedure[]> {
  return cached(`proc:expensive:${limit}`, async () => {
    const r = await db.prepare(
      `SELECT * FROM procedures
       WHERE national_avg_medicare_payment IS NOT NULL AND national_avg_medicare_payment > 0
       ORDER BY national_avg_medicare_payment DESC LIMIT ?1`
    ).bind(limit).all<Procedure>();
    return r.results;
  });
}
...
export function getHighestMarkupProcedures(db: D1Database, limit: number = 10): Promise<Procedure[]> {
  return cached(`proc:markup:${limit}`, async () => {
    const r = await db.prepare(
      `SELECT *, ROUND(national_avg_submitted_charge / NULLIF(national_avg_medicare_payment, 0), 1) as markup_ratio
       ...
       ORDER BY markup_ratio DESC LIMIT ?1`
    ).bind(limit).all<Procedure>();
    return r.results;
  });
}
...
export function getWidestPriceRange(db: D1Database, limit: number = 10): Promise<Procedure[]> {
  return cached(`proc:range:${limit}`, async () => {
    const r = await db.prepare(
      `SELECT *, (price_range_high - price_range_low) as price_spread
       ...
       ORDER BY price_spread DESC LIMIT ?1`
    ).bind(limit).all<Procedure>();
    return r.results;
  });
}
...
export function getCategoryBySlug(db: D1Database, slug: string): Promise<ProcedureCategory | null> {
  return cached(`cat:${slug}`, async () => {
...
export function getProcedureStatePrices(db: D1Database, procedureCode: string): Promise<ProcedureStatePriceWithState[]> {
  return cached(`psp:${procedureCode}`, async () => {
    const r = await db.prepare(
...
export function getProcedureStatePrice(db: D1Database, procedureCode: string, state: string): Promise<ProcedureStatePriceWithState | null> {
  return cached(`psp:${procedureCode}:${state}`, async () => {
    const r = await db.prepare(
      `SELECT p.*, s.name as state_name
       FROM procedure_state_prices p
       JOIN states s ON p.state = s.abbr
...
export function getStateProcedurePrices(db: D1Database, state: string, limit: number = 50): Promise<Array<ProcedureStatePrice & { description: string; slug: string; category: string }>> {
  return cached(`state:procs:${state}:${limit}`, async () => {
    const r = await db.prepare(
...
export function getMostExpensiveInState(db: D1Database, state: string, limit: number = 10): Promise<Array<ProcedureStatePrice & { description: string; slug: string }>> {
  return cached(`state:expensive:${state}:${limit}`, async () => {
    const r = await db.prepare(
...
export function getCheapestInState(db: D1Database, state: string, limit: number = 10): Promise<Array<ProcedureStatePrice & { description: string; slug: string }>> {
  return cached(`state:cheapest:${state}:${limit}`, async () => {
    const r = await db.prepare(
      `SELECT p.*, pr.description, pr.slug
       FROM procedure_state_prices p
       JOIN procedures pr ON p.procedure_code = pr.code
...
export function getHospitalBySlug(db: D1Database, slug: string): Promise<Hospital | null> {
  return cached(`hosp:${slug}`, async () => {
    const r = await db.prepare('SELECT * FROM hospitals WHERE slug = ?1').bind(slug).first<Hospital>();
    return r;
  });
}

export function getHospitalsByState(db: D1Database, state: string): Promise<Hospital[]> {
  return cached(`hosp:state:${state}`, async () => {
    const r = await db.prepare(
...
export function getProcedureStateProviders(db: D1Database, procedureCode: string, state: string, limit: number = 25): Promise<ProviderProcedureWithDetails[]> {
  return cached(`psp:providers:${procedureCode}:${state}:${limit}`, async () => {
    const r = await db.prepare(
...
// --- Search ---
export function searchProcedures(db: D1Database, query: string, limit: number = 20): Promise<Procedure[]> {
  const q = `%${query}%`;
...
export function searchHospitals(db: D1Database, query: string, limit: number = 20): Promise<Hospital[]> {
  const q = `%${query}%`;
...
// --- Cache Warming ---
export async function warmQueryCache(db: D1Database, batchSize: number = 10, pauseMs: number = 500): Promise<void> {
  console.log('[cache] Starting query cache warming...');
  const start = Date.now();

  // Priority 1: Homepage data
  await getStats(db);
  await getTopProcedures(db, 10);
  await getAllCategories(db);
  await getMostExpensiveProcedures(db, 10);
  await getHighestMarkupProcedures(db, 10);
  await getWidestPriceRange(db, 10);

  // Priority 2: Indexes
  await getAllStates(db);
  await getAllProcedures(db);

  // Priority 3: Top procedure profiles (batch with pauses)
  const topProcs = await getTopProcedures(db, 50);
  for (let i = 0; i < topProcs.length; i += batchSize) {
    const batch = topProcs.slice(i, i + batchSize);
    await Promise.all(batch.map(async (p) => {
      await getProcedureStatePrices(db, p.code);
    }));
    if (i + batchSize < topProcs.length) {
